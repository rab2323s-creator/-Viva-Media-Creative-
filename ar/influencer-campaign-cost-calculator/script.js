...
function audienceMultiplier(percent){
  if (!percent) return 0.96;
  if (percent < 20) return 0.78;
  if (percent < 40) return 0.90;
...
function productionCost(level, market, videos, creators){
  ...
  const units = Math.max(1, Math.ceil(videos / Math.max(1,creators)));
  const factor = MARKETS[market].production;
...
function gather(){
  return {
    ...
    followers:clamp(num('followers',50000),1000,100000000), views:clamp(num('averageViews',0),0,100000000),
...
function calculate(d){
  const tier=tierForFollowers(d.followers), rate=TIER_RATES[tier], pm=PLATFORM_MULT[d.platform], marketFactor=MARKETS[d.market].factor;
  ...
  const quality=clamp(engagementMultiplier(d.engagement,tier)*audienceMultiplier(d.audience)*INDUSTRY[d.industry]*SEASON[d.season]*DEMAND[d.demand]*GOAL[d.goal],0.72,1.58);
  ...
  let confidence=48;
  const assumptions=[];
  confidence+=d.views?18:0; if(!d.views) assumptions.push('لم تُدخل متوسط المشاهدات؛ تم الاعتماد أكثر على فئة المتابعين.');
  ...
  confidence=clamp(confidence,45,94);
...
function buildMessage(o){
  const d=o.d;
  return [`مرحبًا Viva Media Creative،`,`أريد عرض سعر رسمي لحملة مؤثرين.`,`الدولة: ${MARKETS[d.market].label}`,`المنصة: ${d.platform}`,`عدد المؤثرين: ${d.creators}`,`فئة المتابعين: ${tierLabel(o.tier)} (${d.followers.toLocaleString('ar-EG')})`,`إجمالي المخرجات: ${o.totalDeliverables}`,`التقدير: ${fmt(o.totalMin,d.market)} — ${fmt(o.totalMax,d.market)}`,`التقدير الأقرب: ${fmt(o.totalLikely,d.market)}`].join('\n');
}
function optimize(){
